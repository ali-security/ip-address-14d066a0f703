...
describe('v6', () => {
  ...
  describe('a v4-in-v6 address with a zero-padded octet', () => {
    ...
    it('is rejected by fromAddress4', () => {
      // Before the fix new Address4('012.0.0.1') parsed, and correctForm()
      // rendered it as '12.0.0.1' while every resolver reads the octal
      // notation as 10.0.0.1 -- so the mapped address named a different host
      // than the one that would be connected to.
      should.Throw(() => Address6.fromAddress4('012.0.0.1'), AddressError);
      should.equal(Address6.fromAddress4('12.0.0.1').correctForm(), '::ffff:c00:1');
    });
